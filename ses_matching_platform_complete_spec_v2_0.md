...
- 案件・人材の一覧では、登録から7日以内のレコードにNEW印を表示する。
...
|---|---|
| ホーム | KPI、対応待ち、警告、通知 |
| 案件 | 自社案件、公開案件検索、登録・更新 |
| 人材 | 自社管理人材、公開人材検索、登録・更新 |
| マッチング | 案件→人材、人材→案件 |
| 商談管理 | 商談申込みの送信・受信、承認、商談状況 |
| 面談 | 日程、結果、条件調整 |
| 契約・稼働 | 基本契約、個別契約、更新、終了 |
| 請求 | 手数料、請求書、入金、返金 |
| メッセージ | 企業間連絡 |
| 企業間関係 | 取引先、一社下、営業委任 |
| レポート | 成約、稼働、売上、精度 |
...
`受領 → 原本保存 → ウイルス検査 → OCR → 種別分類 → PII検出 → 匿名化 → LLM正規化 → JSON検証 → 人手確認 → 確定DB`
...
- 面談回数、回答期限
- 進行状態（応募中／商談中／成約／終了。商談・契約に連動して自動更新、手動上書き可。§20.4）
- 元文書、ルート、変更履歴
...
| 契機 | 案件の進行状態 | 人材の稼働状態 |
|---|---|---|
| 商談開始（双方承認） | 応募中→商談中 | 紹介中→商談中 |
| 見送り・辞退 | 進行中の他商談がなければ商談中→応募中 | 進行中の他商談がなければ商談中→紹介中 |
| 契約の相互締結（成約） | →成約 | 紹介中・商談中→成約 |
| 稼働開始 | （変更なし） | →稼働中 |
| 稼働前キャンセル | 他に有効な契約がなければ成約→応募中 | 他に有効な契約がなければ成約→紹介中 |
