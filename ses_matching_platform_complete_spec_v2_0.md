...
11. LLMはゼロデータ保持・学習オプトアウト契約の事業者を利用する（取込原文の送信は2026-08-19に許可 §25.2）。
...
#### 6.4.1 申込フォーム（2026-08-15追加）

- 入力項目: 企業形態（法人/個人事業者）、企業名、法人番号（任意・13桁）、所在地（**必須・都道府県から入力**。管轄法務局の重複判定に使用するためサーバー側でも都道府県の存在を検証する）、代表者氏名、メールアドレス（確認コードで検証。6桁・15分有効・使用後破棄）、パスワード、規約・基本契約・プライバシーポリシー同意
- 申込画面の横に「企業登録のルール」「担当者の登録」「運用の流れ」「禁止事項」の説明パネルを常設表示する
- 申込者は代表（OWNER ロール）として登録され、企業は審査待ち（APPLIED）で作成。運営承認で開通し、承認日時が新規企業30日間手数料無料の起点となる

#### 6.4.2 重複登録の防止（2026-08-15追加）

判定は決定的（正規化による）チェックを先に行い、通過した場合のみ LLM 類似判定を行う。

| チェック | 一致条件 | 動作 |
|---|---|---|
| メールアドレス | 登録済みメールと一致（DB一意制約） | 登録NG |
| 法人番号 | 13桁の完全一致（入力時のみ） | 登録NG |
| 会社名＋管轄法務局 | 正規化した会社名が一致 かつ 管轄法務局（都道府県）が一致（または所在地の完全一致） | 登録NG |
| 会社名のみ | 正規化した会社名が一致（管轄は不一致） | 警告 → 確認の上で登録可 |
| 所在地のみ | 正規化した所在地が完全一致（会社名は不一致） | 警告 → 確認の上で登録可 |
| 管轄のみ | 都道府県のみ一致 | 警告なし（通常の申込） |
| LLM類似判定 | 名称の表記ゆれ・略称・所在地の近接から同一企業の可能性ありと判定 | 警告 → 確認の上で登録可（LLM未設定・障害時はスキップ） |

- 正規化: 会社名は全角半角・大文字小文字・空白・法人格表記（株式会社/(株)/㈱/前株後株 等）のゆれを吸収。所在地は丁目/番地/号・漢数字・ハイフン類のゆれを吸収。管轄法務局は所在地の都道府県で近似する（商業・法人登記の管轄が都道府県本局に集約されているため）
- 警告時は一致した既存企業名と一致理由（同名/同じ所在地）を表示し、「別企業として申込む」で続行、「入力を修正する」で差し戻す。既存企業への参加は代表からの招待に誘導する
- 警告を確認して申込んだ事実は監査ログ（`CompanyApplied` の `duplicateWarningConfirmed`）に記録し、運営審査で参照できる
- LLM 類似判定の検出は `CompanyDuplicateDetected`（判定理由付き）として監査ログに記録する
- NG・警告の判定は純粋関数として実装し、単体テストで境界動作を固定する（§34）
...
1担当者に複数ロールを付与できる。企業オーナーの画面・メール・エラーメッセージ上の表記は「代表」に統一する（2026-08-15。内部ロールコード OWNER・API は変更しない）。
...
`受領 → 原本保存 → (ウイルス検査: MVP省略) → OCR（画像・画像PDFのみ） → 複数案件の分割 → 種別分類 → LLM正規化 → JSON検証 → 人手確認 → 確定DB`
（PII検出・匿名化工程は2026-08-19に廃止 §25.2）
...
（2026-08-19: LLM送信時のマスキングは廃止 §25.2。本節のマスクは他社への公開表示
（Level 1 匿名性・匿名要約）における非開示項目の定義として維持する）
...
候補選定では、終了・成約（ENDED/CONTRACTED）した案件と、成約・稼働中（CONTRACTED/WORKING）の人材をマッチング対象から除外する（検索フィルターの対象案件・対象人材の判定も同様。2026-08-13追加、成約案件は2026-08-14追加）。詳細画面のマッチング実行は、終了案件・成約/稼働中人材では非表示、成約案件・非公開（下書き・停止中）の案件・人材では無効化して理由を表示する。

必須スキルの充足は既定で100%（全て充足）を要求するが、マッチング実行画面の「必須スキル適合」条件（100%／90%／80%／70%／50%以上）でしきい値を下げ、一部不足の候補も抽出できる（API: `minRequiredSkillRatio` 0.5〜1）。不足した必須スキルは不足条件として表示し、スコアは充足率に連動する（2026-08-14追加）。
...
### 25.2 送信禁止（2026-08-19 全面撤廃）

**LLM送信禁止は2026-08-19に全面撤廃した（郭指示）。** 取込書類のテキストは原文のまま
LLMへ送信し、氏名は取込時のLLM抽出対象とする（確認画面で人手確定 §9.2）。
前提条件はLLM事業者とのゼロデータ保持・学習オプトアウト契約（§25.4）。

撤廃されても変わらないもの:
- 公開画面の匿名性: Level 1 では氏名・実額単価・企業実名を表示しない（§10）。匿名要約に氏名・企業実名を書かないことはLLMプロンプトで指示し、人手確認で担保する
- 段階開示（双方承認まで実名非開示）・相互承認前メッセージの連絡先検出（§21）
- 採否・契約判断をLLMだけで自動決定しない（§25.1）

経緯: 当初は氏名・生年月日・連絡先・住所番地・顔写真・企業実名・実額契約金額・健康/家族情報・
在留証憑を送信禁止としていた。国籍・性別・年齢は2026-08-05に撤廃（§15）、
企業申込の重複判定の例外を2026-08-15に追加、2026-08-19に全面撤廃。

### 25.3 方式

`LLM → JSON検証 → 人手確認`（2026-08-19: PII検出・トークン置換・匿名化検査の工程を廃止し、原文を送信する）
...
| Method | Path | 概要 |
|---|---|---|
| POST | `/companies/applications` | 企業申込（重複警告時は `duplicateWarning` を返し、確認フラグ付き再送で確定） |
| POST | `/companies/applications/email-verification` | 申込メールの確認コード送付 |
...
| GET/PUT/DELETE | `/operations/companies` | 運営: 企業の一覧・修正（企業情報の全項目: 名称・種別・法人番号・所在地・派遣許可情報）・削除 |
...
| 2026-08-19 | LLM送信禁止を全面撤廃（§25.2、郭指示）。取込パイプラインのPII検出・匿名化工程を廃止し原文をLLMへ送信。氏名を取込時のLLM抽出対象とし確認画面で人手確定。公開画面の匿名性・段階開示は維持 |
...
| 2026-08-14 | 一覧の新着印を「登録から24時間以内・小さな赤丸アイコン」に変更（§8.1）。案件抽出で習得予定技術（取得技術・勉強期間あり等）を必須スキルにしない補正を追加（§9.2）。マッチング実行に「必須スキル適合」条件（充足率しきい値）を追加（§19.1） |
| 2026-08-15 | 企業申込の重複防止を追加: 会社名（正規化）＋管轄法務局（都道府県）の両方一致はNG、社名のみ・所在地完全一致のみは警告→確認の上で登録可、管轄のみ一致は警告なし、LLM類似判定は警告扱い（§6.4.2）。所在地を必須化（都道府県から入力。§6.4.1）。申込画面に登録ルール・担当者登録・運用方法の説明パネルを追加（§6.4.1）。企業重複判定に限る企業名・所在地のLLM送信例外を明記（§25.2）。画面表記「オーナー」を「代表」に統一（§7.2）。運営コンソールの企業修正を全項目対応に拡張（§28） |
