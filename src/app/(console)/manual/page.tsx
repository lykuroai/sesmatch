    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="space-y-5">
        ...
        <Section id="ingestions" title="取込・取込履歴">
          <p>
            メール本文・ファイル・撮影画像から案件票やスキルシートを取り込み、個人情報を匿名化したうえで内容を自動抽出します。
            取込の実行は案件・人材画面の「取込」ボタンから行います。
          </p>
          <Steps
            items={[
              <>
                ファイル（Excel/Word/PDF/画像等）をアップロード、またはテキストを貼り付けて取込を開始します。
                スキャン済みの画像PDFや写真はOCRで文字起こしします（手書き・不鮮明な写真は精度が落ちます）。
                スマホでは「カメラで撮影して取込」から1ページずつ撮影し、複数ページをまとめて1件として取り込めます。
              </>,
              <>
                取込は受付後すぐに完了応答が返り、解析（OCR・匿名化・AI抽出）は裏で進みます（1分ほど）。
                1つの貼り付けに複数の案件（【案件名】見出しが2つ以上）が含まれていると、案件ごとに自動で分割されます。
              </>,
              <>システムが個人情報を検出・匿名化し、匿名化済みテキストから項目を抽出します（氏名・連絡先・原本画像はそのまま外部に送りません）。</>,
              <>「取込履歴」の「人手確認待ち」で抽出値を人が確認・修正して確定します。確定した内容が案件・人材として登録されます。</>,
            ]}
          />
          <p>
            取込履歴では「人手確認待ち」とそれ以外の履歴が分かれて表示され、各取込の状態（解析中／人手確認待ち／確定済み／失敗）を確認できます。失敗した取込は「再実行」できます。
          </p>
        </Section>
