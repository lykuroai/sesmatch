...
- `src/server/pipeline/` — 取込パイプライン。`llm.ts`（`LlmGateway` interface + モック）/ `llm-claude.ts`（Claude API 実装。JSON Schema による構造化出力 + zod 検証、トークン数を監査ログへ記録 §25.4）。`pii.ts` はメッセージの連絡先検出（§21）のみ（取込時の匿名化は2026-08-19廃止）
...
SES企業間で流通する案件情報と人材情報を、メール・ファイル・画面入力から収集し、**LLM正規化** を経て一元管理するB2Bプラットフォーム。全契約ユーザーは企業・事業者（テナント）で、案件⇄人材の双方向マッチングからエントリー、双方承認、面談、契約、稼働、月次手数料請求までを一貫して扱う。
...
- **取込パイプライン**: `受領 → 原本保存 → ウイルス検査 → OCR → 種別分類 → LLM正規化 → JSON検証 → 人手確認 → 確定DB`（PII検出・匿名化工程は2026-08-19廃止 §25.2）。原文・LLM抽出値・人が確認した確定値は分離保存する。
...
- **LLM への送信制約**: **2026-08-19に全面撤廃（郭指示）**。取込テキストは原文のままLLMへ送信し、氏名・国籍は取込時のLLM抽出対象（確認画面で人手確定）。前提はゼロデータ保持・学習オプトアウト契約（§25.4）。公開画面の匿名性（Level 1 で氏名・実額・企業実名を出さない、匿名要約に書かない）と段階開示は維持。採否・契約判断を LLM だけで自動決定しない。
...
- **Phase 1**: 企業コンソール、企業/担当者/ロール、案件・人材管理、取込、LLM正規化、双方向マッチング、同意管理
